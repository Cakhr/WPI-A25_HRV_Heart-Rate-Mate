...
const pairIcon = (route: RelativePathString) => {
	...
	switch (route as String) {
		...
		case ('/indexFour'):
			return(<></>);
		default:
			console.log('This should not happen lol, no icon matched to route [' + route + ']')
			return(<></>);
	}
}
...
export const NavButton = (route: RelativePathString) => {
	return (
		<Link href={route} asChild>
			<Pressable className={'flex-1 items-center justify-items-center bg-card rounded-2xl p-1 shadow-lg transition-all'}>
